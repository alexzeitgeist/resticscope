...
// browseRowPaths returns each row's unique path in order.
...
// sorterFixture returns canonical database order with file attributes chosen so
// name, size, and modification-time sorting produce distinct permutations.
...
// Recursive directory sizes must not move directories below larger files.
...
// Equal sort keys use the case-insensitive name tie-break.
...
// Unknown modification times follow known times and are name-ordered together.
func TestSortedBrowseRowsModifiedZeroTimeLast(t *testing.T) {
	known := time.Date(2026, 4, 4, 0, 0, 0, 0, time.UTC)
	in := []model.BrowseEntry{
		{Path: "/zzz-unknown.txt", Name: "zzz-unknown.txt"},
		{Path: "/known.txt", Name: "known.txt", ModTime: known},
		{Path: "/aaa-unknown.txt", Name: "aaa-unknown.txt"},
	}
...
// A unique-path final tie-break makes output independent of input order.
...
// Name mode aliases its input; other modes copy it and preserve the canonical
// cache order.
