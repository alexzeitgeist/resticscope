// Package model defines resticscope's shared domain types. It is the dependency
// leaf; persistence and privacy policy belong to consuming layers.
package model

import "time"

// Snapshot contains the subset of `restic snapshots --json` used by
// resticscope. Unknown fields are ignored for forward compatibility.
...
// SnapshotSummary contains summary statistics returned inline for snapshots
// created by restic 0.17 and later, avoiding a separate stats call. Snapshot
// uses a pointer to distinguish a missing summary from a genuine zero-byte
// result.
